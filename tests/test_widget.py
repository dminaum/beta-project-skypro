...
from src.widget import formatting_time, masking_operation
...
def test_masking_operation_valid(input_number: str, expected_result: str):
    assert masking_operation(input_number) == expected_result


@pytest.mark.parametrize(
    "invalid_number",
    ["Maestro 12345678901234567", "MasterCard 987654321098765", "Счет 1234567890", "Счет 123456789012345678901"],
)
def test_masking_operation_invalid(invalid_number: str):
    assert masking_operation(invalid_number) == "Неправильно введен номер карты/счета. Пожалуйста, попробуйте еще раз"


@pytest.mark.parametrize(
    "time_, expected_result",
    [
...
def test_formatting_time(time_: str, expected_result: str):
